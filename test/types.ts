import { Mimos, MimosEntry } from '../lib/index.ts';
...
expect.type<Mimos<{ custom: boolean }>>(new Mimos<{ custom: boolean }>({ override: { 'test/html': { compressible: false, predicate, custom: true } } }));
...
expect.type<Readonly<MimosEntry>>(mimos.type('text/html'));
expect.type<Readonly<MimosEntry & Partial<{ custom: boolean }>>>(new Mimos<{ custom: boolean }>().type('text/html'));
...
expect.type<Readonly<MimosEntry> | {}>(mimos.path('/my/file.html'));
expect.type<Readonly<MimosEntry & Partial<{ custom: boolean }>> | {}>(new Mimos<{ custom: boolean }>().path('/my/file.html'));
